...
SoftwareSerial Slave(10, 11);         // Указываем выводы RO и RI
...
void setup() 
{
  ...
  Serial.println("Slave!");           // Отправка текста  
  Slave.begin(19200);                 // Открываем последовательную связь
...
void loop() { 
  while (Serial.available()) {        // Проверяем,поступили ли данные 
    ...
    Slave.write(val);                 // Отправка данных
  }
 
  digitalWrite(DE_RE,LOW);            // Устанавливаем DE_RE в LOW  
  while(Slave.available()) {          // Проверяем,поступили ли данные  
    val = Slave.read();               // Копируем данные val
